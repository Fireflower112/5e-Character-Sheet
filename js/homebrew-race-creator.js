...
DndSheet.pages.showHomebrewRaceModal = (raceToEdit = null) => {
    ...
    // Attach handlers immediately after rendering
    ...
    const saveAndClose = () => {
        ...
        DndSheet.stores.character.saveHomebrewRace(raceData);

        if (!raceToEdit && document.getElementById('add-subrace-toggle').checked) {
            DndSheet.pages.showHomebrewSubraceModal(raceData.name);
        } else {
            closeModal();
        }
    };
